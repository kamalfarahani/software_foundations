...
Proof.
  ...
  apply functional_extensionality.
  intros x'.
  destruct (eqb_stringP x x') as [Heq | Hneq].
  - rewrite Heq. rewrite t_update_eq. reflexivity.
  - apply t_update_neq. apply Hneq.
Qed.
(** [] *)
...
Proof.
  ...
  intros x'.
  unfold t_update.
  destruct (eqb_stringP x1 x') as [Heq | Hneq].
  - destruct  (eqb_stringP x2 x') as [Heq' | Hneq'].
    + assert (x1 = x2) as contra.
      { apply trans_eq with (y := x'). assumption. symmetry. assumption. }
      symmetry in contra.
      apply H in contra.
      destruct contra.
    + reflexivity.
