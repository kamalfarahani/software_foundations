...
Lemma rev_app: forall (X: Type) (l: list X) (x: X),
  rev (l ++ [x]) = [x] ++ rev l.
Proof.
  intros X l x.
  induction l as [| h t IH].
...
Lemma fold_map_app : forall X Y (f: X -> Y) (l: list X) (x: X),
...
Proof: We proceed by induction on the structure of the list l.

Base case: l = []
In this case, length [] = 0, so n = 0 by our assumption. 
We need to show that @nth_error X [] 0 = None.
By the definition of nth_error, nth_error [] 0 = None. So the base case holds.

Inductive case: l = a :: l' for some element a and list l'
Induction hypothesis: For all m, if length l' = m, then @nth_error X l' m = None.

We know that length (a :: l') = S (length l'), so if length (a :: l') = n, then n = S (length l').
By the definition of nth_error:
  nth_error (a :: l') n = if n =? 0 then Some a else nth_error l' (pred n)

Since n = S (length l'), we know n ≠ 0, so n =? 0 evaluates to false.
Therefore, nth_error (a :: l') n = nth_error l' (pred n).

Since n = S (length l'), we have pred n = length l'.
By our induction hypothesis with m = length l', we know nth_error l' (length l') = None.

Therefore, nth_error (a :: l') n = nth_error l' (pred n) = nth_error l' (length l') = None.
...
Definition mult (n m : cnat) : cnat :=
  let adder := 
      fun (X' : Type) (f' : X' -> X') => (plus n zero) X' f'
  in
  fun (X : Type) (f : X -> X) (x : X) => (m X (adder X f) x).
...
Definition exp (n m : cnat) : cnat :=
  ...
  let n_multiplier := 
      fun (X': Type) (f': X' -> X') => mult n one X' f' 
  in
  (m (X -> X) (n_multiplier X) f) x.
